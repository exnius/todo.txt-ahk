...
LV_SetColor(Index="", TextColor="", BackColor="", Redraw=1)
{
	...
	; LV_SetColor had an error that treated the colors as 0xBBGGRR.
	; FixColorFormat() changes 0xRRGGBB into 0xBBGGRR to fix this.
	TextColor := FixColorFormat(TextColor)
	BackColor := FixColorFormat(BackColor)
...
; LV_SetColor had an error that treated the colors as 0xBBGGRR.
; FixColorFormat() changes 0xRRGGBB into 0xBBGGRR to fix this.
FixColorFormat(color) 
{
If (RegExMatch(color, "i)(0x)([0-9A-F][0-9A-F])([0-9A-F][0-9A-F])([0-9A-F][0-9A-F])", colorPart))
	color := colorPart1 . colorPart4 . colorPart3 . colorPart2
Return color
}
